  ...
  const panel = (
    <div className={cn(
      ...
      <div className="flex items-center justify-between">
        ...
        {value && (
          <Button variant="ghost" size="sm" className="h-6 px-2 text-[10px] text-destructive hover:bg-destructive/10" onClick={clearImage}>
            Remove Image
          </Button>
        )}
      </div>
      ...
      {value ? (
         <div className="relative rounded-xl overflow-hidden bg-muted/30 border border-border/50 aspect-video flex items-center justify-center">
            {!imagePreviewError ? (
              <img src={value} alt="Preview" className="w-full h-full object-contain" onError={() => setImagePreviewError(true)} />
            ) : (
      ...
      {!value && (
        <>
          <div className="flex items-center gap-3">
            ...
            <span className="text-[9px] text-muted-foreground font-medium uppercase tracking-widest">OR</span>
            <div className="h-px flex-1 bg-border/40" />
          </div>
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <Input
                ...
                onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), applyImageUrl())}
                placeholder={placeholder}
                className="h-9 text-xs bg-muted/30 border-border/50 focus-visible:ring-1 pl-8"
              />
            </div>
            <Button variant="secondary" size="sm" className="h-9 text-xs shrink-0 px-4" onClick={applyImageUrl}>
              Apply
            </Button>
          </div>
        </>
    ...
    <div ref={rootRef} className="relative">
      <div 
        ...
        className={cn(
          ...
          value ? "border-primary/40 shadow-sm" : "border-transparent hover:bg-primary/10 hover:text-primary text-muted-foreground",
