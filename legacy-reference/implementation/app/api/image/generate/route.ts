...
function validateImageGenerateBody(body: ImageGenerateBody):
  ...
  if (!prompt || prompt.length > 2000) {
    return {
      ok: false,
      response: jsonError('INVALID_PROMPT', 'prompt is required and must be <= 2000 chars.', 400),
    };
  }

  const negativePrompt =
    typeof body.negative_prompt === 'string' ? body.negative_prompt.trim() : undefined;
  if (negativePrompt && negativePrompt.length > 2000) {
    return {
      ok: false,
      response: jsonError('INVALID_NEGATIVE_PROMPT', 'negative_prompt must be <= 2000 chars.', 400),
    };
  }
  ...
  if (imageUrl) {
    ...
    if (!isHttp && !isBase64DataUrl) {
      return {
        ...
        response: jsonError(
          'INVALID_IMAGE_URL',
          'image_url must be a public URL or data:image base64.',
          400
        ),
      };
    }
  ...
  if (!sizeMatch) {
    return {
      ok: false,
      response: jsonError('INVALID_SIZE', 'size must be in WIDTHxHEIGHT format.', 400),
    };
  }
  ...
  if (width < 64 || width > 2048 || height < 64 || height > 2048) {
    return {
      ok: false,
      response: jsonError('INVALID_SIZE', 'image size must be between 64 and 2048.', 400),
    };
  }
  ...
  if (steps !== undefined && (!Number.isInteger(steps) || steps < 1 || steps > 100)) {
    return {
      ok: false,
      response: jsonError('INVALID_STEPS', 'steps must be an integer between 1 and 100.', 400),
    };
  }
  ...
  if (guidance !== undefined && (guidance < 1.5 || guidance > 20)) {
    return {
      ok: false,
      response: jsonError('INVALID_GUIDANCE', 'guidance must be between 1.5 and 20.', 400),
    };
  }
  ...
  if (seed !== undefined && (!Number.isInteger(seed) || seed < 0 || seed > 2_147_483_647)) {
    return {
      ok: false,
      response: jsonError('INVALID_SEED', 'seed must be a positive 32-bit integer.', 400),
    };
  }
  ...
  if (typeof body.loras === 'string') {
    ...
    if (entries.length < 1 || entries.length > 6) {
      return {
        ok: false,
        response: jsonError('INVALID_LORAS', 'loras object must contain 1 to 6 entries.', 400),
      };
    }
    ...
    for (const [key, value] of entries) {
      ...
      if (!repo || repo.length > 200) {
        return {
          ok: false,
          response: jsonError('INVALID_LORAS', 'loras repo id is invalid.', 400),
        };
      }
      if (typeof value !== 'number' || value < 0 || value > 1) {
        return {
          ok: false,
          response: jsonError('INVALID_LORAS', 'loras weight must be between 0 and 1.', 400),
        };
      }
    ...
    if (Math.abs(totalWeight - 1) > 0.02) {
      return {
        ok: false,
        response: jsonError('INVALID_LORAS', 'multi-loras weights must sum to 1.0.', 400),
      };
    }
    loras = parsedLoras;
  }
...
export async function POST(req: NextRequest) {
  ...
  try {
    const parsed = await parseJsonBody<ImageGenerateBody>(
      req,
      apiConfig.imageGenerate.maxBodyBytes
    );
